...
/// Attribute names that the server emits synthetically from each object's
/// built-in fields. A stored attribute with one of these names is dropped
/// during `get_attributes` so the wire response never has two entries
/// with the same name (one stored, one synthetic).
...
/// KMIP attribute names that may never be added, modified, or deleted by
/// a client. Centralised so the trait impls and dispatch handlers cannot
/// drift out of sync.
...
/// Return true if `name` (case-insensitive) names one of the
...
pub trait KmipKeyStore: Send + Sync {
    ...
    /// Atomically insert `obj` under `id` only if no object currently
    /// exists with that ID. Returns `Ok(())` on insert, or
    /// `Err(KmipResultReason::ObjectNotFound)` on collision (the
    /// not-found masking is intentional — see [`process_register`] for
    /// the rationale).
    ///
    /// Default implementation forwards through [`KmipKeyStore::register`]
    /// so existing backends keep compiling; the [`InMemoryKeyStore`]
    /// override uses `DashMap::entry()` for a true atomic check-and-insert.
    fn insert_if_absent(&self, id: String, obj: KmipStoredObject) -> Result<(), KmipResultReason> {
        self.register(id, obj.object_type, obj.key_material, obj.attributes)
            .map(|_| ())
    }
...
/// Audit follow-up: a wall-clock jump backwards must NOT let the
/// `modified`/`created` timestamps regress, which would otherwise allow
/// an out-of-order replay to look "newer" than a request that arrived
/// first. We expose a monotonic-floor variant that uses CAS to ratchet
/// the highest observed epoch second so all timestamps emitted by the
/// in-memory store are weakly monotonic across calls. Wall-clock
/// jitter forward is honoured; jitter backwards is clamped to the last
/// observed value.
static NOW_EPOCH_FLOOR: AtomicU64 = AtomicU64::new(0);
...
fn now_epoch() -> u64 {
    let wall = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_secs();
    // Standard CAS loop to install the maximum of (wall, current floor).
    let mut current = NOW_EPOCH_FLOOR.load(Ordering::Relaxed);
    loop {
        let next = wall.max(current);
        match NOW_EPOCH_FLOOR.compare_exchange_weak(
            current,
            next,
            Ordering::Relaxed,
            Ordering::Relaxed,
...
impl KmipKeyStore for InMemoryKeyStore {
    ...
    fn get_attributes(&self, id: &str) -> Option<Vec<KmipAttribute>> {
        ...
        let obj = entry.value();
        // Build a Vec<KmipAttribute> from the stored HashMap plus synthetic
        // attributes derived from the object's built-in fields.
        //
        // Audit follow-up: a stored attribute whose name collides with one
        // of the server-synthesised attributes (State, Object Type, Initial
        // Date, Last Change Date) would otherwise shadow the synthetic
        // value on the wire. We strip such collisions and emit a warning so
        // operator dashboards can spot a misbehaving Register caller.
        let mut attrs: Vec<KmipAttribute> = obj
            .attributes
            .iter()
            .filter_map(|(name, value)| {
                if SYNTHETIC_NAMES.iter().any(|s| s.eq_ignore_ascii_case(name)) {
                    tracing::warn!(
                        target: "craton_hsm_kmip::ops",
                        attr = %name,
                        kmip.unique_id = %id,
                        "dropping stored attribute whose name shadows a synthetic attribute"
                    );
                    None
                } else {
                    Some(KmipAttribute {
    ...
    fn register(
        ...
        let ts = now_epoch();
        // Use the DashMap entry() API to atomically check-and-insert, eliminating
        ...
        if inserted {
            ...
            // Audit follow-up: return `ObjectNotFound` rather than
            // `InvalidMessage` so a colliding Register cannot be used as
            // a key-existence oracle across tenants. The dispatcher-level
            // ACL check (`process_register`) is consulted first; when the
            // ACL would have allowed the caller, this branch still has to
            // refuse the overwrite — exposing `InvalidMessage` here would
            // distinguish "ID already taken" from "ID free" in a way that
            // leaks information about other tenants' identifiers.
    ...
    fn add_attributes(
        ...
        //
        // Defence in depth: even though `process_add_attribute` already
        // refuses immutable names before reaching this trait method, we
        // re-check here so a caller that constructs a request bypassing
        // the dispatcher (e.g. directly invoking the trait method from
        // a custom server) still cannot mutate the immutable set.
        for (k, _) in new_attrs.iter() {
            let trimmed = k.trim();
            if trimmed.is_empty() || trimmed != k || k.chars().any(|c| c.is_whitespace() && c != ' ') {
    ...
    fn modify_attributes(
        ...
        // Defence in depth: refuse immutable names at the trait layer
        // even though `process_modify_attribute` already filters.
    ...
    fn delete_attributes(
        ...
        // Defence in depth: refuse immutable names at the trait layer
        // even though `process_delete_attribute` already filters.
        for n in names {
    ...
    fn insert_if_absent(&self, id: String, obj: KmipStoredObject) -> Result<(), KmipResultReason> {
        ...
        if inserted {
            self.index_insert(ot, &new_id);
            Ok(())
        } else {
            // Existence-oracle masking: return ObjectNotFound, matching
            // `register()`'s collision branch.
            Err(KmipResultReason::ObjectNotFound)
        }
    }
}
...
///
/// Audit follow-up: the previous implementation returned `false` early on
/// length mismatch, which leaks the byte-length of the stored owner string
/// through timing. We now fold both byte strings through a single XOR
/// reduction over `max(len_a, len_b)`, ORing the length-mismatch bit into
/// the final [`subtle::Choice`] so the rejection path runs in time
/// proportional to the longer string regardless of whether the lengths
/// agree or differ.
fn ct_str_eq(a: &str, b: &str) -> bool {
    ...
    // Start with the length-mismatch bit: `Choice` is 1 when the lengths
    // do NOT match. We OR this in at the end so it cannot be short-
    // circuited by the byte loop.
    let len_mismatch: Choice = ((aa.len() as u64) ^ (bb.len() as u64)).ct_eq(&0u64);
    // `len_mismatch == 1` means "lengths are equal". Invert later via
    // negation when we combine. To keep everything in `Choice`, we keep
    // it as "lengths_equal" and AND it with the byte-wise compare.
    ...
    let bytes_equal: Choice = byte_diff.ct_eq(&0u8);
    bool::from(len_mismatch & bytes_equal)
}
...
fn check_owner_acl_for_request(
    ...
    check_owner_acl_with_options(
        obj,
        &request.caller_identity,
        request.strict_owner_acl,
    )
}
...
pub fn process_get(
    ...
    match key_store.get(id) {
        Some(obj) if obj.state == KmipObjectState::Destroyed => {
            KmipResponse::error(KmipResultReason::ObjectNotFound, "object has been destroyed")
        }
        Some(obj) => {
            ...
            resp.key_material = obj.key_material;
            resp
        }
...
pub fn process_activate(
    ...
    acl_checked_transition(key_store, id, &request.caller_identity, "Activate", move |id, store| {
        match store.activate(id) {
            Ok(()) => {
                tracing::info!(
                    kmip.op = "Activate",
                    kmip.unique_id = %id,
...
pub fn process_revoke(
    ...
    let caller = request.caller_identity.clone();
    acl_checked_transition(key_store, id, &request.caller_identity, "Revoke", move |id, store| {
...
pub fn process_destroy(
    ...
    let caller = request.caller_identity.clone();
    acl_checked_transition(key_store, id, &request.caller_identity, "Destroy", move |id, store| {
            ...
            "Object Type" => {
                if let KmipAttributeValue::Enum(v) = &attr.value {
                    match KmipObjectType::from_u32(*v) {
                        ...
                        None => {
                            // Audit follow-up: reject unknown Object Type
                            // enum values rather than silently falling back
                            // to SymmetricKey. A client that sends an
                            // unrecognised value must learn its request was
                            // malformed, not see it auto-coerced.
                            return KmipResponse::error(
                                KmipResultReason::InvalidField,
                                &format!("Register: unknown Object Type enum value 0x{v:08x}"),
                            );
                        }
                    }
                }
...
/// Default cap on the number of object IDs a single `Locate` response may
/// return. Without a cap a malicious filter (e.g. one that matches every
/// object in a very large store) could force the server to allocate a
/// response large enough to OOM the wire encoder. The truncation point
/// is reported back to the caller via a synthetic `Locate-Truncated`
/// attribute so they know to refine their filter.
pub const LOCATE_MAX_RESULTS_DEFAULT: usize = 1000;

/// Process a Locate operation: search objects by attribute filters and
/// post-filter the result set through the owner-ACL.
///
/// Results are clipped at [`LOCATE_MAX_RESULTS_DEFAULT`]; a clip causes
/// a synthetic `Locate-Truncated` attribute to appear in the response so
/// the caller can refine the filter rather than silently lose ids.
pub fn process_locate(
    request: &KmipRequest,
    key_store: &dyn KmipKeyStore,
) -> KmipResponse {
    ...
    // avoid cloning every object on the hot path.
    let caller = request.caller_identity.clone();
    ...
    if filtered.len() > LOCATE_MAX_RESULTS_DEFAULT {
        filtered.truncate(LOCATE_MAX_RESULTS_DEFAULT);
        truncated = true;
    }
    ...
    if truncated {
        // Surface the clip via a synthetic attribute so callers can detect
        // and refine their filter rather than silently dropping ids.
        resp.attributes.push(KmipAttribute {
            name: "Locate-Truncated".to_string(),
            value: KmipAttributeValue::Integer(LOCATE_MAX_RESULTS_DEFAULT as i32),
        });
    }
    resp
}
...
pub fn process_derive_key(
    ...
    // Audit follow-up: hold both the input keying material and the
    // derived output in `Zeroizing` so a panic between the HKDF call
    // and the final `key_store.create(...)` cannot leave plaintext
    // bytes on the heap. The IKM is cloned out of the stored object
    // since the store handed us an owned `Vec<u8>` (its own copy is
    // independently zeroed on Destroy).
    let ikm: zeroize::Zeroizing<Vec<u8>> = match &ikm_obj.key_material {
        Some(m) if !m.is_empty() => zeroize::Zeroizing::new(m.clone()),
        _ => {
            return KmipResponse::error(
                KmipResultReason::CryptographicFailure,
    ...
    let mut okm: zeroize::Zeroizing<Vec<u8>> = zeroize::Zeroizing::new(vec![0u8; want]);
    ...
    // Clone the bytes out of the Zeroizing wrapper one last time so the
    // store can take ownership; the wrapper is dropped immediately after,
    // wiping the local copy.
    let material: Vec<u8> = (*okm).clone();
    drop(okm);
    let new_id = key_store.create(KmipObjectType::SymmetricKey, material, attrs);
    let mut resp = KmipResponse::success(&new_id);
    resp.object_type = Some(KmipObjectType::SymmetricKey);
    resp
}
...
mod tests {
    ...
    fn create_request(mut attrs: Vec<KmipAttribute>) -> KmipRequest {
        ...
        if !attrs
            .iter()
            .any(|a| a.name == "Cryptographic Length")
        {
            attrs.push(KmipAttribute {
    ...
    fn get_attributes_returns_stored_attrs() {
        ...
        let has_name = resp.attributes.iter().any(|a| {
            a.name == "Name"
                && a.value == KmipAttributeValue::Text("test-key".to_string())
        });
    ...
    fn get_attributes_includes_synthetic_state() {
        ...
        let has_state = resp.attributes.iter().any(|a| {
            a.name == "State"
                && a.value == KmipAttributeValue::Text("PreActive".to_string())
        });
    ...
    fn register_duplicate_id_fails() {
        ...
        let resp = process_register(&req, &store);
        assert_eq!(resp.status, KmipResultStatus::OperationFailed);
        // Audit follow-up: collisions now report ObjectNotFound rather than
        // InvalidMessage so a client cannot use Register as an existence
        // oracle for IDs assigned by other tenants.
        assert_eq!(resp.reason, Some(KmipResultReason::ObjectNotFound));
    }
    ...
    fn test_activate_acl_blocks_wrong_caller() {
        ...
        let mut attrs = HashMap::new();
        attrs.insert("owner".to_string(), KmipAttributeValue::Text("alice".to_string()));
        let id = store.create(KmipObjectType::SymmetricKey, vec![0u8; 32], attrs);
    ...
    fn test_destroy_zeroizes_key_material() {
        ...
        let obj = store.get("zeroize-key").unwrap();
        assert!(obj.key_material.is_none(), "key material must be cleared after destroy");
        assert_eq!(obj.state, KmipObjectState::Destroyed);
    }
    ...
    fn destroy_active_key_rejected() {
        ...
        assert_eq!(destroy_resp.reason, Some(KmipResultReason::PermissionDenied));
    ...
    // -----------------------------------------------------------------------
    // Audit-followup hardening tests
    // -----------------------------------------------------------------------
    ...
    /// never observe the second value strictly below the first, even
    /// after a backwards wall-clock jolt (modelled by the floor).
    #[test]
    fn now_epoch_is_weakly_monotonic() {
        let a = now_epoch();
        // Spin a handful of calls; none may decrease.
        let mut prev = a;
        for _ in 0..1000 {
            let cur = now_epoch();
            assert!(
                cur >= prev,
                "now_epoch regressed: prev={prev} cur={cur}"
            );
            prev = cur;
        }
    }
    ...
    /// Modify must refuse the immutable set at the dispatcher AND if
    /// somebody bypasses the dispatcher and calls the trait method
    /// directly the trait impl must still refuse.
    #[test]
    ...
    /// `insert_if_absent` claims the id atomically and returns
    /// `ObjectNotFound` on collision (the existence-oracle masking).
    #[test]
    fn insert_if_absent_atomic_and_masks_collision() {
        let store = make_store();
        let obj = KmipStoredObject {
        ...
        assert!(store.insert_if_absent("stable-id".into(), obj.clone()).is_ok());
        let again = store.insert_if_absent("stable-id".into(), obj);
        assert_eq!(again, Err(KmipResultReason::ObjectNotFound));
    }
    ...
    fn get_attributes_drops_synthetic_shadow() {
        let store = make_store();
        let mut attrs = HashMap::new();
        // A malicious Register that managed to stash a "State" attribute
        // must NOT see it echoed back instead of the real synthetic State.
        attrs.insert(
            "State".to_string(),
        ...
        let states: Vec<&KmipAttribute> =
            out.iter().filter(|a| a.name == "State").collect();
        assert_eq!(states.len(), 1, "exactly one State attribute must appear");
        // It must be the synthetic version, NOT "Hijacked".
        assert_eq!(
            states[0].value,
    ...
    fn locate_truncates_at_default_cap() {
        let store = make_store();
        // Insert one more than the cap; all owned by alice.
        ...
        let trunc = resp
            .attributes
            .iter()
            .any(|a| a.name == "Locate-Truncated");
        assert!(trunc, "expected Locate-Truncated synthetic attribute");
    }

    /// DeriveKey wraps IKM and OKM in `Zeroizing`; we cannot read raw
    /// stack memory in safe Rust, but we can at least verify that the
    /// derived key is registered with the requested length and a fresh
    /// id (smoke test that the Zeroizing rewrite didn't break the
    /// happy path).
    #[test]
    fn derive_key_succeeds_and_returns_new_id() {
        let store = make_store();
        // Register an input keying material.
        let req = KmipRequest {
            operation: KmipOperation::Register,
