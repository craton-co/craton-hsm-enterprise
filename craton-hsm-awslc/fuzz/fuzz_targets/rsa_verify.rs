...
//! Fuzz RSA PKCS#1 v1.5 signature verification on the aws-lc-rs backend.
//!
//! Input layout:
...
//!
//! We short-circuit when the declared lengths don't fit the buffer. The
//! target never panics regardless of whether the inputs are syntactically
//! valid — validation that returns `Err` is fine; a panic is a bug.
